...
- ✅ Users can pick a display currency (USD, EUR, GBP, besides the default BRL) in Settings. All monetary values across the app are converted from the underlying BRL data using exchange rates fetched from a free public API (with a static approximate fallback if the API is unavailable) — data is still stored in BRL. `MoneyInput` (transaction entry) intentionally stays in raw BRL to avoid corrupting stored data — [constants/currencies.ts](constants/currencies.ts), [service/exchangeRateService.ts](service/exchangeRateService.ts), [stores/currency.ts](stores/currency.ts), [composables/useMoney.ts](composables/useMoney.ts), [composables/useFormatCurrency.ts](composables/useFormatCurrency.ts), [pages/settings/index.vue](pages/settings/index.vue), [app.vue](app.vue) (e.g. USD, EUR, GBP, etc.) with exchange rates and currency conversion.