...
import {
  // Calendar, // Commented out as it's also a component name, will use reka-ui's directly or alias if needed
  ChevronLeft,
  ChevronRight,
...
import { Calendar as CalendarIcon } from 'lucide-vue-next'; // Alias Calendar icon
...
const stringToCalendarDate = (dateString: string | null): CalendarDate | null => {
  ...
  try {
    ...
    console.error(t('error_converting_date'), e) // Internationalized
    return null
  }
}
...
// const locale = ref('pt-BR') // Replaced by useI18n locale
const formatter = useDateFormatter(locale.value, { // locale.value will be reactive from useI18n
  month: 'long',
  year: 'numeric'
})
...
const firstMonth = ref(
  createMonth({
    ...
    locale: locale.value, // use reactive locale
    fixedWeeks: true,
    weekStartsOn: 0,
  }),
...
watch(locale, (newLocale: any) => {
  formatter.value = useDateFormatter(newLocale, { month: 'long', year: 'numeric' }); // Update formatter instance
...
async function fetchTransactions() {
  ...
  try {
    ...
    const userId = "user_2rSVhqngUjGL0zVLiBXfXixKAG3" // Hardcoded User ID - NOTE: Needs to be addressed
    if (!userId) {
      transactionsStore.setError(t('error_user_not_authenticated')) // Internationalized
      return
    }
    ...
    console.error(t('error_fetching_transactions_console'), err) // Internationalized
    transactionsStore.setError(t('error_fetching_transactions_user')) // Internationalized
  } finally {
...
const formattedDateRange = computed(() => {
  if (!value.value.start) return t('select_date_placeholder') // Internationalized

  // Ensure formatter is updated if locale changes
  const currentFormatter = useDateFormatter(locale.value, { day: "2-digit", month: "2-digit", year: "numeric" });

  const startFormatted = currentFormatter.format(toDate(value.value.start))

  if (!value.value.end) return startFormatted

  const endFormatted = currentFormatter.format(toDate(value.value.end))
    ...
    <Popover>
      <PopoverTrigger as-child>
        ...
        >
          ...
          <CalendarIcon class="mr-2 h-4 w-4" aria-hidden="true" /> <!-- Aliased Icon -->
          {{ formattedDateRange }}
        </Button>
      </PopoverTrigger>
          ...
          <div class="flex flex-col gap-y-4 mt-4 sm:flex-row sm:gap-x-4 sm:gap-y-0">
            <div class="flex flex-col gap-4">
              <div class="flex items-center justify-between">
                ...
                <div :class="cn('text-sm font-medium')" role="heading" aria-level="2">
                  {{ formatter.format(toDate(firstMonth.dateObj)) }}
                </div>
                <button
                  :class="cn(buttonVariants({ variant: 'outline' }), 'h-7 w-7 bg-transparent p-0 opacity-50 hover:opacity-100')"
            ...
            <div class="flex flex-col gap-4">
              <div class="flex items-center justify-between">
                ...
                <div :class="cn('text-sm font-medium')" role="heading" aria-level="2">
                   {{ formatter.format(toDate(secondMonth.dateObj)) }}
                </div>
                <button
                  :class="cn(buttonVariants({ variant: 'outline' }), 'h-7 w-7 bg-transparent p-0 opacity-50 hover:opacity-100')"
                  :aria-label="t('next_month')"
