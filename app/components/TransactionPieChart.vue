...
// Refs para os dados
const typesPercentage = ref<Record<string, number>>({});
...
if (process.client) {
  onMounted(async () => {
    ...
    const themesStoreInstance = useThemeStore();

    theme.value = themesStoreInstance.theme;
    typesPercentage.value = transactionsStoreInstance.typesPercentage;
...
const chartData = computed(() => {
  return Object.entries(typesPercentage.value).map(([type, value]) => ({
