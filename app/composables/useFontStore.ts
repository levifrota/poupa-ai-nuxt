...
export const useFontStore = defineStore("font", () => {
  ...
  // Inicializar apenas se estamos no cliente
...
export default defineNuxtPlugin(() => {
  ...
  if (import.meta.client) {
    ...
    if (fontStore.fontFamily) {
      const fontClass = `font-${fontStore.fontFamily}`;
      document.documentElement.classList.add(fontClass);
    }

    if (fontStore.fontSize) {
      const sizeClass = `text-${fontStore.fontSize}`;
